...
### 🖥️ Web-интерфейс
- **Matrix Dashboard** — стильный дашборд в стиле "Матрица" (зелёный на чёрном, анимация дождя)
- **Управление прокси** — запуск/остановка/выход через браузер
- **Настройки** — изменение всех параметров без перезапуска
- **Логи в реальном времени** — WebSocket для live-логов
- **Docker-ready** — один контейнер с web-интерфейсом и прокси
...
### Web-интерфейс (Matrix Dashboard)

Запуск с веб-дашбордом для управления прокси через браузер:

```bash
python -m tor_https_bridge --web --web-port 8080
```

Откройте `http://localhost:8080` в браузере.

**Возможности дашборда:**
- Matrix-стиль: зелёный на чёрном, анимация "дождя" из символов
- **START** — запуск прокси с текущими настройками
- **STOP** — остановка прокси
- **EXIT** — полное завершение (остановка прокси + закрытие сервера)
- Настройка всех параметров: порты, таймауты, stealth mode
- Логи в реальном времени (WebSocket)

**CLI аргументы для web:**

| Аргумент | По умолчанию | Описание |
|---|---|---|
| `--web` | `false` | Включить web-дашборд |
| `--web-host` | `0.0.0.0` | Адрес web-сервера |
| `--web-port` | `8080` | Порт web-сервера |
